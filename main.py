...
def processFile(update,bot,message,file,thread=None,jdb=None):
    ...
    if client:
        ...
        bot.editMessageText(message,'⚠️𝙴𝚛𝚛𝚘𝚛 en la 𝚗𝚞𝚋𝚎⚠️')
    ...
    try:
        ...
        if '/token_on' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'⚠️ Error en el comando /token_on estado de Tokenizar⚠️')
            return
        if '/token_off' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'⚠️Error en el comando /token_off estado de Tokenizar⚠️')
            return
        if '/cloud' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
        ...
        if '/view_proxy' in msgText:
            try:
                ...
                if getUser:
                    proxy = getUser['proxy']
                    msg = '🧬El proxy usado actualmente es:👇🏻\n'
        ...
        #         bot.editMessageText(message,'⚠️Error y causas🧐\n1-Revise su Cuenta\n2-Servidor Desabilitado: '+client.path)
            ...
            #    bot_token = '5205765255:AAE9LW0N8wbfB_AvuFHY30oNQo4h8WwlPPA'
